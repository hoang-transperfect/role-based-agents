...
description: >
  ...
  may reference foundation tokens and foundation components (Text, Icon) but no other atoms.
  Follows the project's atom spec template: frontmatter (name, version, description, link,
  dependencies), Anatomy (Structure + Details), Appearance (Props + Variants + State + Tokens),
  Content (Copy + Empty state + Overflow + Case treatment + Allowed characters + Placeholder +
  Number and date formatting + Text expansion), Accessibility (ARIA table + Focus management +
  Keyboard table + Mouse + Touch + Screen reader table + Internationalization), and a Checklist
  (Design + Engineering + QA). Invoke once per atom listed in ds-audit.md, after
  designer-ds-foundation has defined the token layer and designer-ds-foundation-component has
  specced Text and Icon. Atoms must be specced before any molecule or DS organism that depends
  on them. Produces one component-spec.md per atom under design-system/atoms/.
---

# designer-ds-atom

An atom is the smallest indivisible UI element in the design system — Button, Input, Checkbox,
Badge, Avatar, Spinner, Tooltip, etc. It may reference **foundation tokens** and **foundation
...
- Foundation files exist and define the token names this atom will reference. If a needed token
  ...
  before proceeding.
- Both `<real_project_path>/design-system/foundation-components/icon/component-spec.md` and
  `<real_project_path>/design-system/foundation-components/text/component-spec.md` must exist.
  If either is missing, stop and route to `designer-ds-foundation-component` first — do not
  proceed with any atom.
...
Check that both `<real_project_path>/design-system/foundation-components/icon/component-spec.md`
and `<real_project_path>/design-system/foundation-components/text/component-spec.md` exist. If
either is missing, stop: "Foundation components must be specced before any atom. Let's spec
{missing one} first." Do not proceed until both files exist.
...
link:
  - {tool-name}: {tool-link}
dependencies:
  - foundation-components: N/A  # list Text and/or Icon if used in anatomy
---
# {Component Name}
